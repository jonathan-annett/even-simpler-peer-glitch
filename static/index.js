/*
global offerPeer,answerPeer,pluginPeer,propertyInspectorPeer,mockSDApi,mockPIApi,browserAppPeer
*/
...
document.querySelector("#test1").onclick = function () {
  peer.send(JSON.stringify("test Message " + Math.random().toString()));
};
...
document.querySelector("#test2").onclick = function () {
  peer2.send(JSON.stringify("test Message " + Math.random().toString()));
};
...
plugin_peer.on("data", function (data) {
  document.querySelector("#plug_in").value =
    typeof data === "object" ? JSON.stringify(data, undefined, 4) : data;
});

document.querySelector("#plug_test").onclick = function () {
  plugin_peer.send("test Message " + Math.random().toString());
};

const pi_peer = propertyInspectorPeer(
  document.querySelector("#pi_button"),
  mockPIApi()
);
...
app_peer.on("data", function (data) {
  document.querySelector("#app_in").value =
    typeof data === "object" ? JSON.stringify(data, undefined, 4) : data;
});

document.querySelector("#app_test").onclick = function () {
  app_peer.send("test Message " + Math.random().toString());
};
